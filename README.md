...
- **🎨 AI 改图**：捕捉当前视野（可调 90°–120°，比屏幕看到的更大一圈）送给模型编辑，结果按相机几何**重投影回所有被覆盖的立方体面**并羽化融合，跨面无缝。支持选模型（wan2.7 / qwen-image 系列）、参考图（选文件或 Ctrl+V 粘贴，提示词用「图1」指当前画面、「图2」指参考图）。每次编辑作为一组版本记录，可整组切换/恢复原图，可叠加编辑。结果存在 `assets/edits/`。`?viewtest&capyaw=45` 可自测重投影几何。
